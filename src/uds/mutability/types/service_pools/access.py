"""``servicepool.access``: set the access calendar rules of a service pool.

Relation action type over ``CalendarAccess`` rows (option A: ``calendars``
describes the FINAL set, never a delta). Execution diffs it against the
live rows keyed by ``calendar_id`` and applies the minimal
create/edit/delete sequence through the canonical
``services_pools/{uuid}/access`` detail surface.
...
calendar that already has a rule on the pool. There is no DB unique
...
def _access_rows(pool: models.ServicePool) -> list[AccessRow]:
    ...
    return [
        ...
        for rule in pool.calendarAccess.select_related("calendar").order_by("calendar__uuid")
    ]


class ServicePoolAccess(mutability_base.MutableActionType):
    """Proposal: set the complete desired access calendar rules of a service pool."""

    type_id = "servicepool.access"

    title = "Propose service pool access calendars"
    description = (
        "Propose the complete desired set of access calendar rules of a service pool "
        "(when the pool may be used). The 'calendars' field is the FINAL set, not a "
        "delta: rules missing from it will be removed, new ones added, and "
        "access/priority adjusted. Each rule is one calendar with access ALLOW or "
        "DENY and a priority (lower wins). Use get_servicepool_access to see the "
        "current rules and the uuids of the calendars you can assign. An empty list "
        "removes every rule (the pool then depends only on its fallback access). "
    ...
    noun = "Service pool"
    # Discovery without the pool uuid is useless here: the agent must see
    # the current rules before it can propose the final set.
    ...
    def validate_values(
        ...
        if isinstance(target, models.ServicePool):
            ...
            if len(set(live)) != len(live):
                errors.append(
                    "the service pool currently has duplicated access rules for the same calendar "
    ...
    def snapshot_values(self, target: db_models.Model, names: collections.abc.Iterable[str]) -> JsonObject:
        ...
        wanted = set(names)
        return {"calendars": _access_rows(pool)} if "calendars" in wanted else {}

    @typing.override
    def fingerprint(self, target: db_models.Model) -> str:
        rows = _access_rows(typing.cast(models.ServicePool, target))
    ...
    def read_title(self) -> str:
        return "Read service pool access calendars"

    @typing.override
    def read_description(self) -> str:
        return (
            "Read the current access calendar rules of one service pool: every rule with its "
            "calendar uuid (to use in a servicepool.access.set proposal), name, access "
    ...
    def read(self, target_uuid: str) -> JsonObject:
        ...
        result["current_rules"] = [
            {**row, "calendar_name": names.get(str(row["calendar_id"]), "")} for row in _access_rows(pool)
        ]
        return result
    ...
    async def op_set(self, action: "models.FlowAction", request: ExtendedHttpRequestWithUser) -> str:
        ...
        def _plan() -> tuple[str, list[AccessOperation]]:
            ...
            rows = _access_rows(pool)
            current = {row["calendar_id"]: row for row in rows}
            if len(current) != len(rows):
                raise rest_exceptions.RequestError(
                    f'Service pool "{pool.name}" has duplicated access rules for the same calendar; '
            ...
            rule_uuids = {
                rule.calendar.uuid: rule.uuid for rule in pool.calendarAccess.select_related("calendar")
            }
        ...
        return f'Service pool "{pool_name}" access calendars: {created} created, {modified} modified, {removed} removed'
