"""``servicepool.fallback``: change the fallback access policy of a service pool.
...
active). It is a plain scalar on the pool, written through the canonical
``services_pools/{uuid}/fallback_access`` custom method (POST), and it is
deliberately its own family: the ``servicepool.update`` PUT does not
accept it and the access-rules set does not touch it, so the CAS stays
per-concern. There is no ``read`` override: the current value is already
published by the curated ``get_servicepool_fallback_access`` tool.
"""
...
class ServicePoolFallback(mutability_base.MutableActionType):
    """Proposal: change the fallback access policy of a service pool."""

    type_id = "servicepool.fallback"

    title = "Propose service pool fallback access"
    description = (
        "Propose the fallback access policy of a service pool: what happens when no "
        "access calendar rule applies (ALLOW lets everyone in, DENY keeps everyone "
        "out, unless a calendar rule says otherwise). Read the current value with "
        "get_servicepool_fallback_access. The proposal does NOT apply anything: it "
    ...
    def snapshot_values(self, target: db_models.Model, names: collections.abc.Iterable[str]) -> JsonObject:
        ...
        wanted = set(names)
        return {"fallback": pool.fallbackAccess} if "fallback" in wanted else {}

    @typing.override
    def fingerprint(self, target: db_models.Model) -> str:
        pool = typing.cast(models.ServicePool, target)
        return item_etag({"fallback": pool.fallbackAccess}, ["fallback"])
    ...
    async def op_update(self, action: "models.FlowAction", request: ExtendedHttpRequestWithUser) -> str:
        ...
        pool_name = await sync_to_async(_pool_name, thread_sensitive=True)()
        ...
        return f'Service pool "{pool_name}" fallback access set to {fallback}'
