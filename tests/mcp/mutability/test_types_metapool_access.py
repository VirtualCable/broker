"""``metapool.access.set`` / ``metapool.fallback.update``: access surface.

The rules family mirrors ``metapool.member`` (rows with per-row data,
keyed by the natural key: the calendar), the fallback is a scalar family
executed through the master custom POST
``meta_pools/{uuid}/fallback_access``.
"""
...
from uds.mutability.types.meta_pools.access import MetaPoolAccess
from uds.mutability.types.meta_pools.fallback import MetaPoolFallback
...
def _access() -> typing.Any:
    factory = registry_get("metapool.access.set")
    assert factory is not None
    return factory()


def _fallback() -> typing.Any:
    factory = registry_get("metapool.fallback.update")
    assert factory is not None
    return factory()


class MetaPoolAccessRegistryTest(FlowTestCase):
    def test_is_registered(self) -> None:
        found = registry_get("metapool.access.set")
        assert found is not None
        self.assertIs(type(found()), MetaPoolAccess)
        self.assertIn("metapool.access.set", all_type_ids())
    ...
    def test_is_target_scoped(self) -> None:
        self.assertIs(MetaPoolAccess.target_scoped_fields, True)

    def test_read_view_publishes_rules(self) -> None:
        meta_pool = _metapool()
        calendar = _calendar()
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar, access="ALLOW", priority=3
        )
        # The read generator instantiates the family unbound (reads are
        # not operations).
        view = MetaPoolAccess().read(meta_pool.uuid)
        self.assertEqual(view["entity_type"], "metapool.access")
        self.assertEqual(
            view["current_rules"],
            [{"calendar_id": calendar.uuid, "access": "ALLOW", "priority": 3, "calendar_name": calendar.name}],
        )

    def test_write_only_family_is_metapool_fallback(self) -> None:
        # The fallback current value is already published by the plain GET
        # fallback_access surface: no generated read for it.
        self.assertFalse(MetaPoolFallback.readable())
        self.assertTrue(MetaPoolAccess.readable())


class MetaPoolAccessFieldsTest(FlowTestCase):
    def test_single_editlist_field_with_calendar_choices(self) -> None:
        ...
        defs = _access().field_definitions("metapool")
    ...
    def test_valid_complete_set(self) -> None:
        ...
        calendar = _calendar()
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar, access="DENY", priority=1
        )
        rows = [{"calendar_id": calendar.uuid, "access": "DENY", "priority": 1}]
        self.assertEqual(_access().validate_values("metapool", {"calendars": rows}, meta_pool), [])
        # Empty set is valid: it means "remove every rule"
        self.assertEqual(_access().validate_values("metapool", {"calendars": []}, meta_pool), [])

    def test_missing_calendars_field_rejected(self) -> None:
        errors = _access().validate_values("metapool", {"other": 1})
    ...
    def test_row_shape_errors(self) -> None:
        ...
        for row, fragment in bad_rows:
            errors = _access().validate_values("metapool", {"calendars": [row]})
    ...
    def test_duplicate_calendar_id_rejected(self) -> None:
        ...
        errors = _access().validate_values("metapool", {"calendars": [row, dict(row)]})
    ...
    def test_live_duplicated_rows_rejected(self) -> None:
        ...
        calendar = _calendar()
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar, access="ALLOW", priority=0
        )
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar, access="DENY", priority=7
        )
        rows = [{"calendar_id": calendar.uuid, "access": "ALLOW", "priority": 0}]
        errors = _access().validate_values("metapool", {"calendars": rows}, meta_pool)
        self.assertTrue(any("duplicated access rules" in e for e in errors))


class MetaPoolAccessCasTest(FlowTestCase):
    def test_snapshot_canonical_rows(self) -> None:
        meta_pool = _metapool()
        calendar_a = _calendar("calendar-a")
        calendar_b = _calendar("calendar-b")
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar_a, access="ALLOW", priority=1
        )
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar_b, access="DENY", priority=0
        )
        snapshot = _access().snapshot_values(meta_pool, ["calendars"])
        expected = sorted(
            [
    ...
    def test_fingerprint_tracks_rules(self) -> None:
        ...
        rule = meta_pool.calendarAccess.first()
...
class MetaPoolAccessExecuteTest(FlowTestCase):
    def _run(self, meta_pool: models.MetaPool, calendars: list[dict[str, typing.Any]]) -> mock.Mock:
        action = self._action(
            self._flow(),
            action_type="metapool.access.set",
        ...
        with mock.patch("uds.mutability.types.meta_pools.access.RestProxy", proxy_cls):
    ...
    def _rows(meta_pool: models.MetaPool) -> list[dict[str, typing.Any]]:
        return [
            ...
            for rule in meta_pool.calendarAccess.all()
        ]
    ...
    def test_diff_applies_delete_create_update_in_order(self) -> None:
        ...
        meta_pool = _metapool()
        models.CalendarAccessMeta.objects.create(meta_pool=meta_pool, calendar=kept, access="ALLOW", priority=0)
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=dropped, access="DENY", priority=0
        )
        ...
        for call in calls:
            ...
            self.assertEqual(target.path, "meta_pools/{uuid}/access")
            self.assertIs(target.parent.handler, MetaPools)
            self.assertEqual(call[1]["parent_uuid"], meta_pool.uuid)

        delete_call = calls[0]
        dropped_rule_uuid = models.CalendarAccessMeta.objects.get(meta_pool=meta_pool, calendar=dropped).uuid
        ...
        update_call = calls[2]
        kept_rule_uuid = models.CalendarAccessMeta.objects.get(meta_pool=meta_pool, calendar=kept).uuid
    ...
    def test_equal_set_is_a_noop(self) -> None:
        ...
        calendar_b = _calendar("calendar-b")
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar_a, access="ALLOW", priority=0
        )
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=calendar_b, access="DENY", priority=1
        )
    ...
    def test_empty_set_removes_everything(self) -> None:
        meta_pool = _metapool()
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=_calendar("calendar-a"), access="ALLOW"
        )
        models.CalendarAccessMeta.objects.create(
            meta_pool=meta_pool, calendar=_calendar("calendar-b"), access="DENY"
        )
    ...
    def test_live_duplicated_rows_abort_before_touching_rest(self) -> None:
        ...
        with self.assertRaises(rest_exceptions.RequestError):
            self._run(meta_pool, [{"calendar_id": calendar.uuid, "access": "ALLOW", "priority": 0}])


class MetaPoolFallbackTest(FlowTestCase):
    def test_is_registered(self) -> None:
        found = registry_get("metapool.fallback.update")
        assert found is not None
        self.assertIs(type(found()), MetaPoolFallback)
        self.assertIn("metapool.fallback.update", all_type_ids())

    def test_single_choice_field(self) -> None:
        defs = _fallback().field_definitions("metapool")
    ...
    def test_valid_values(self) -> None:
        meta_pool = _metapool()
        self.assertEqual(_fallback().validate_values("metapool", {"fallback": "DENY"}, meta_pool), [])
        self.assertEqual(_fallback().validate_values("metapool", {"fallback": "ALLOW"}, meta_pool), [])

    def test_invalid_values(self) -> None:
        errors = _fallback().validate_values("metapool", {})
        self.assertTrue(any("fallback is required" in e for e in errors))
        errors = _fallback().validate_values("metapool", {"fallback": "MAYBE"})
    ...
    def test_snapshot_and_fingerprint_track_the_scalar(self) -> None:
        ...
        self.assertEqual(
            action_type.snapshot_values(meta_pool, ["fallback"]),
            {"fallback": meta_pool.fallbackAccess},
        )
        base = action_type.fingerprint(meta_pool)
        meta_pool.fallbackAccess = "DENY"
        meta_pool.save(update_fields=["fallbackAccess"])
        self.assertNotEqual(base, action_type.fingerprint(meta_pool))
    ...
    def test_execute_posts_the_master_custom_method(self) -> None:
        ...
        action = self._action(
            self._flow(),
            action_type="metapool.fallback.update",
            target_uuid=meta_pool.uuid,
        ...
        with mock.patch("uds.mutability.types.meta_pools.fallback.RestProxy", proxy_cls):
        ...
        self.assertEqual(target.args, (meta_pool.uuid, "fallback_access"))
